...
# ==========================================
# 🌲 NATIVE PINECONE & EMBEDDER SETUP (LAZY LOAD)
# ==========================================
...
def get_embedder():
    """Lazy load embedder only when first needed — prevents HF Space startup hang"""
    global embedder
    if embedder is None:
        try:
...
if PINECONE_API_KEY:
    try:
        ...
        logger.info("🌲 Native Pinecone connected! (Embedder will load on first use)")
    except Exception as e:
...
async def save_vision_memory(image_b64: str, user_text: str, response_data: dict):
    ...
    try:
        ...
        if pinecone_index and _embedder:
            try:
                def sync_upsert():
                    vector = _embedder.encode(mem_content).tolist()
...
async def save_deep_memory(req: DeepMemorySaveReq):
    ...
    try:
        ...
        if pinecone_index and _embedder:
            try:
                def sync_upsert():
                    vector = _embedder.encode(req.content).tolist()
...
def search_vector(req: SearchRequest):
    ...
    if not pinecone_index or not _embedder:
        raise HTTPException(status_code=503, detail="Pinecone or Embed Model not configured")
    try:
        vector = _embedder.encode(req.query).tolist()
